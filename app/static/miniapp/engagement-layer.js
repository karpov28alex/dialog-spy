(() => {
  ...
  async function auth() {
    ...
    const response = await fetch('/api/auth/telegram', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({init_data:tg.initData}), cache:'no-store'});
  ...
  function stories(data){
    ...
    const signalStories=signals.slice(0,4).map(item=>({icon:item.icon||'🧠',title:String(item.title||'Insight').slice(0,16),label:'Phantom заметил',detail:item.text||''}));
  ...
  function render(data){
    ...
    const signature=JSON.stringify(values)+primarySignal(data)+currentStreak+activePeriod+JSON.stringify(signals);
  ...
  async function mount(){if(loading||!isHome())return;loading=true;try{render(await api('/api/intelligence?days=14'));}catch{}finally{loading=false;}}
