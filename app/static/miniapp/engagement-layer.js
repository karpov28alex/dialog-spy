(() => {
  ...
  function stories(data) {
    ...
    const currentStreak = streak(rows);
    const leaders = data?.leaders || {};
    return [
  ...
  function render(data) {
    cachedData = data;
    const values = aggregate(data, activePeriod);
    const currentStreak = streak(activityRows(data));
    ...
    if (!section) {
      section = document.createElement('section'); section.className = 'engagement-pulse';
      app.querySelector('.grid')?.before(section);
    }
    const storyItems = stories(data);
    section.innerHTML = `
      <div class="pulse-head">
        <div><span class="pulse-kicker">PHANTOM RECAP</span><h2>${activePeriod === 'today' ? 'Сегодня в архиве' : 'Неделя в Phantom'}</h2></div>
        <span class="pulse-live"><i></i>${currentStreak > 1 ? ` ${currentStreak} дней` : ' live'}</span>
      </div>
      <div class="recap-tabs"><button data-recap="today" class="${activePeriod==='today'?'active':''}">Сегодня</button><button data-recap="week" class="${activePeriod==='week'?'active':''}">7 дней</button></div>
      ...
      <div class="smart-stories">${storyItems.map((item,index)=>`<button type="button" class="smart-story" data-story="${index}"><span>${item.icon}</span><b>${esc(item.title)}</b><small>${esc(item.label)}</small></button>`).join('')}</div>
      <div class="story-detail" hidden></div>
      <button type="button" class="pulse-insight" data-go="stats"><span>${insightLine(data)}</span><strong>→</strong></button>`;
    mounted = true;
  ...
  function animateCounts(root) {
    root.querySelectorAll('[data-count]').forEach(node => {
      const target = Number(node.dataset.count || 0); const start = performance.now();
  ...
  app.addEventListener('click', event => {
    ...
    if (story && cachedData) {
      const item = stories(cachedData)[Number(story.dataset.story || 0)];
      const detail = app.querySelector('.engagement-pulse .story-detail');
      if (detail && item) { detail.hidden=false; detail.innerHTML=`<b>${item.icon} ${esc(item.title)}</b><span>${esc(item.detail)}</span>`; detail.animate([{opacity:.3,transform:'translateY(6px)'},{opacity:1,transform:'translateY(0)'}],{duration:260,easing:'ease-out'}); }
    }
  });

  async function mount() {
    if (loading || !isHome()) return; loading = true;
    try { render(await api('/api/intelligence?days=7')); } catch {} finally { loading=false; }
  }

  let queued=false;
  new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;if(isHome())mount();else mounted=false;});}).observe(app,{childList:true,subtree:true});

  let ticking=false;
  addEventListener('scroll',()=>{if(ticking)return;ticking=true;requestAnimationFrame(()=>{ticking=false;const section=app.querySelector('.engagement-pulse');if(!section)return;const rect=section.getBoundingClientRect();const progress=Math.max(-1,Math.min(1,(innerHeight*.5-rect.top)/innerHeight));section.style.setProperty('--pulse-shift',`${progress*12}px`);section.style.setProperty('--pulse-glow',String(Math.max(0,.7-Math.abs(progress)*.25)));});},{passive:true});
  mount();
})();