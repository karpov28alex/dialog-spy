...
class SearchRepository:
    ...
    @staticmethod
    def _matches(*columns, query: str):
        vector = func.to_tsvector(
            "simple",
            func.concat_ws(" ", *(func.coalesce(column, "") for column in columns)),
        )
    ...
    def _score(*columns, query: str):
        ...
        return (
            func.ts_rank_cd(vector, tsquery) * 2
            + func.greatest(*similarities)
        ).label("score")
        ...
        return list(
            (
                await self._session.execute(
                    statement.order_by(desc(score), desc(Dialog.last_message_at), desc(Dialog.id))
                    .limit(limit)
                )
            ).all()
        )
    ...
    async def messages(
        ...
        return list(
            (
                await self._session.execute(
                    statement.order_by(desc(score), desc(Message.sent_at), desc(Message.id))
                    .limit(limit)
                )
            ).all()
        )
        ...
        return list(
            (
                await self._session.execute(
                    statement.order_by(
                        desc(score),
                        desc(MessageVersion.created_at),
                        desc(MessageVersion.id),
                    ).limit(limit)
                )
            ).all()
        )
    ...
    async def media(
        ...
        return list(
            (
                await self._session.execute(
                    statement.order_by(desc(score), desc(Message.sent_at), desc(Media.id))
                    .limit(limit)
                )
            ).all()
        )
