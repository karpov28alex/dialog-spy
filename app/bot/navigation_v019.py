...
def profile_keyboard(admin: bool = False) -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton(text="📊 Статистика", callback_data="v019:stats")],
        [InlineKeyboardButton(text="⚙️ Настройки", callback_data="v019:settings")],
        [InlineKeyboardButton(text="📖 Инструкция", callback_data="v019:help")],
    ]
    if admin:
...
async def profile(callback: CallbackQuery) -> None:
    ...
    if callback.message:
        ...
        original = profile_module._profile_keyboard
        profile_module._profile_keyboard = lambda: profile_keyboard(False)
        try:
            await _send_profile(callback.message, callback.from_user.id)
        finally:
            profile_module._profile_keyboard = original
...
async def stats(callback: CallbackQuery) -> None:
    ...
    if callback.message:
        ...
        stats_module._stats_keyboard = lambda admin: back_profile(
            [InlineKeyboardButton(text="☀️ Сегодня", callback_data="engagement:recap:1"), InlineKeyboardButton(text="📅 7 дней", callback_data="engagement:recap:7")],
            [InlineKeyboardButton(text="📲 Открыть статистику", web_app=WebAppInfo(url=f"{settings.mini_app_url}?screen=stats"))],
            [InlineKeyboardButton(text="🚀 Поделиться", callback_data="product:share_card")],
        )
        try:
            await _shareable_stats(callback.message, callback.from_user.id)
        finally:
            stats_module._stats_keyboard = original
...
async def settings_screen(callback: CallbackQuery) -> None:
    ...
    if not callback.message or not prefs:
        return
    await _replace(callback)
    enabled, _ = subscription_commerce_config()
    ...
    if enabled:
        rows.append([InlineKeyboardButton(text="💎 Подписка", callback_data="v019:subscription")])
...
async def subscription_screen(callback: CallbackQuery) -> None:
    ...
    if callback.message:
        ...
        await callback.message.answer(text, reply_markup=back_profile(
            [InlineKeyboardButton(text="📄 Оферта", url=offer_url)],
            [InlineKeyboardButton(text="🚫 Отключить автопродление", callback_data="user:subscription:cancel")],
        ))
