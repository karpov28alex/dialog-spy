...
def test_avatar_loader_uses_bounded_visibility_queue() -> None:
    ...
    assert "rootMargin: '320px 0px'" in source
...
def test_visual_refresh_is_loaded_last() -> None:
    ...
    visual = source.index("/app/visual-refresh.css?v=0.15.0")
    archive = source.index("/app/archive-workspace.css?v=0.15.0")
    assert visual > archive
    assert "v0.15.0 · Visual Refresh" in source
